...
export default function MeusAlugueisPage() {
  ...
  const [locacoes, setLocacoes] = useState<LocacaoClienteItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [filtro, setFiltro] = useState<"ativo" | "todos" | "finalizados">(
    "ativo"
  );
  ...
  useEffect(() => {
    async function fetchLocacoes() {
      ...
      try {
        ...
        setLocacoes((data as LocacaoClienteItem[]) || []);
      } catch (error) {
        console.error("Erro inesperado ao buscar aluguéis:", error);
        toast.error("Erro inesperado ao carregar seus aluguéis");
      } finally {
        setLoading(false);
      }
    }
  ...
  const locacoesFiltradas = useMemo(() => {
    if (filtro === "ativo") {
      return locacoes.filter((l) => l.situacao === "ativo");
    }

    if (filtro === "finalizados") {
      return locacoes.filter(
        (l) => l.situacao === "finalizado" || l.situacao === "cancelado"
      );
    }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-4xl p-6 space-y-6 animate-fade-in">
        ...
        <div className="flex gap-2">
          ...
          <Button
            variant={filtro === "finalizados" ? "default" : "outline"}
            className="rounded-[30px]"
        ...
        {loading ? (
          ...
          <div className="rounded-[30px] border border-border bg-card p-8 text-center">
            <p className="text-muted-foreground">
              Sua conta ainda não foi vinculada a um cadastro de cliente. Entre
              em contato com a MGM Sistemas.
            </p>
          </div>
          ...
          <div className="space-y-4">
            {locacoesFiltradas.map((l) => (
              <div
                key={l.id}
                className="rounded-[30px] border border-border bg-card p-6 transition-all hover:border-primary/30"
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <div className="mt-2 space-y-1 text-sm text-muted-foreground">
                      <p>
                        Período: {formatDate(l.data_inicio)} →{" "}
                      ...
                      {l.data_devolucao_real && (
                        <p>
                          Devolvido em: {formatDate(l.data_devolucao_real)}
                        </p>
                      )}
                    </div>
                  </div>
