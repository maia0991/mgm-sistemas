...
export default function UsuariosPage() {
  ...
  async function fetchData() {
    try {
      ...
      setPerfis(perfisWithRole as PerfilComRole[]);
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const { error } = await supabase.functions.invoke("create-locadora-user", {
        body: {
          email: form.email,
      ...
      <div className="animate-fade-in space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button className="rounded-[30px] gap-2">
        ...
        <div className="space-y-3">
          {perfis.map((p) => (
            <div
              key={p.id}
              className="flex items-center justify-between rounded-[30px] border border-border bg-card p-5 transition-all hover:border-primary/30"
            >
