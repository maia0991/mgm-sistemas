...
export default function MeuPerfilPage() {
  ...
  useEffect(() => {
    ...
    supabase.from("clientes").select("*").eq("id", perfil.cliente_id).single().then(({ data }) => {
      setCliente(data);
    });
  }, [perfil]);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-4xl p-6 space-y-6 animate-fade-in">
        <Link to="/meus-alugueis" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors">
        ...
        <div className="rounded-[30px] border border-border bg-card p-6 space-y-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xl font-bold text-foreground">{perfil?.nome || "—"}</p>
            ...
            <div className="space-y-3 border-t border-border pt-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium text-foreground">{cliente.nome_completo}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium text-foreground">{cliente.whatsapp || "—"}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">Endereço da Obra</p>
                  <p className="font-medium text-foreground">{cliente.endereco_obra || "—"}</p>
                </div>
              </div>
            </div>
          ) : (
